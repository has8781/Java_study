...
//								try { //좌석이동을 위한 비밀번호 입력
//									System.out.print(selected_seat);
//									Class.forName("com.mysql.cj.jdbc.Driver");
//									Connection conn =
//									DriverManager.getConnection(url, mysql_user, mysql_password);
//									String sql = "UPDATE seat SET change_count = change_count +1,use_count = use_count+1, use_time = use_time+ "
//											+ "timediff(now(),buytime), " // 현재시간과 결제시 시간 계산 후 시간 더하기
//											+ "use_Status = 0, buytime = null WHERE (seat_number = '" // buytime 초기화
...
//								}
						...
						nmv.join_n.addActionListener(new ActionListener() {	// 확인 버튼 눌렀을 때 
							public void actionPerformed(ActionEvent e) {
								...
								*/
								//<HTML><center>300시간<br/>300,000원<HTML>
								//npn = nmv.Phtf_n.getText(); //비회원 전화번호 저장
								int result_b = JOptionPane.showConfirmDialog(null,	// OptionPaneEx.this : 가운데 출력
										"<HTML><left>입력하신 전화번호가 맞습니까?<br/>" + nmv.Phtf_n.getText() + "<HTML>", "스터디 카페", JOptionPane.YES_NO_OPTION);
								if(result_b == JOptionPane.YES_OPTION) {
									JOptionPane.showMessageDialog(null, "확인되었습니다.", "스터디 카페", JOptionPane.INFORMATION_MESSAGE);
									nmv.setVisible(false);
						...
						if(new_usestatus == 1) { // 새로선택한 좌석의 사용여부가 true 면
							...
							try { 
								...
								String sql = "UPDATE seat SET use_count = use_count + 1, "
										+ "use_Status = 1 WHERE (seat_number = '" + new_selected_seat + "')"; 
	...
	class nonmember_view extends JPanel{	// 비회원 화면 
		...
		/*
		class nmeberlogin implements ActionListener{ // 비회원 로그인
			public void actionPerformed(ActionEvent e) {
				int login = -1;
				try { // 선택한 좌석의 이용횟수 +1 이용여부 1(true)
					Class.forName("com.mysql.cj.jdbc.Driver");
	...
	class nonmember_buy extends JPanel{	// 비회원 구매 화면
		nonmember_view nvi = new nonmember_view();
				...
				public void actionPerformed(ActionEvent e) {
					...
					if(result == JOptionPane.YES_OPTION) {
						try {
							...
							Connection conn =
							DriverManager.getConnection(url, mysql_user, mysql_password);
							String time = "SELECT time FROM ticket WHERE price = 4000"; //시간 받아오기
							String sql = "INSERT INTO nonmember VALUES" + 
									"('"+ nvi.Phtf_n.getText() + "', '" + time +"')"; //비회원 테이블에 값 저장(오류)
							System.out.println(sql);
							Statement stmt = conn.createStatement();
							stmt.executeUpdate(sql);
